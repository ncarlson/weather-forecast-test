...
```
tanzu apps workload create my-weatherforecast-csharp -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/my-weatherforecast-csharp-source \
  --type web
```
...
```
tanzu apps workload get my-weatherforecast-csharp
```
